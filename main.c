...
void button_scheduler_event_handler(void *p_event_data, uint16_t event_size)
{
    ...
    //E:we add handling part to this function where:
    //(nrf_drv_gpiote_pin_t*) - casting as this var type
    //*((nrf_drv_gpiote_pin_t*)p_event_data) pointer to this
}
...
//    button_handler(pin);
    //E:Instead of handling interrupt, we schedule handling, via button_scheduler_event_handler
